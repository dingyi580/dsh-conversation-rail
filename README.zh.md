...
然后重启 `dsh web`（新插件进装配表要重启；之后改代码可以用 `dev_reload_package` 热重载）。

<details>
<summary>从本地目录安装</summary>

把插件放到 `~/.dsh/plugins/dsh-conversation-rail`，往 `~/.dsh/profiles/web/package.json`
加两处：
...
```json
{
  ...
  "dependencies": {
    "dsh-conversation-rail": "link:/Users/<你>/.dsh/plugins/dsh-conversation-rail"
  }
}
```

再把它链进 profile 的 node_modules：
...
```sh
npm run build       # tsc 编 host 入口 + tsdown 把 src/client 打成 lib/client.js
npm run typecheck   # 只做类型校验
```

`lib/` 随仓库提交，从 GitHub 装的时候不需要再构建一遍。

	...
	factory: (require) => {
		...
		*/
		async function fetchOutline(sessionId, signal) {
			const query = "/dsh-conversation-rail/api/outline?sessionId=" + encodeURIComponent(sessionId);
		...
		*/
		async function attemptOutline(query, signal) {
			try {
...
`;
		...
		* @returns 不小于它的档位值。
		*/
		function quantizeMax(maxWeight) {
		...
		function mountRail(doc, deps) {
			...
			*/
			...
			function place() {
				...
				if (key !== rectKey) {
					...
					settledAt = now;
					host.dataset.visible = "0";
				} else host.dataset.visible = now - settledAt >= SETTLE_MS ? "1" : "0";
				if (host.dataset.visible === "0" && settleHandle === null) settleHandle = setTimeout(() => {
					settleHandle = null;
					place();
				}, 240);
			...
			*/
			function draw() {
				...
				turns.forEach((turn, index) => {
					const reusable = used.has(turn.seq) ? void 0 : nodes.get(turn.seq);
					const hit = reusable ?? createHit();
					if (reusable === void 0 && !used.has(turn.seq)) nodes.set(turn.seq, hit);
					used.add(turn.seq);
			...
			*/
			...
			track.addEventListener("mouseover", (event) => {
				...
				if (turn !== void 0) openCard(turn, hit);
			});
		...
		*/
		function label() {
			return (typeof navigator === "undefined" ? "" : navigator.language ?? "").toLowerCase().startsWith("zh") ? "会话小地图" : "Session minimap";
		}
		/**
		...
		*/
		function apply(ctx) {
			...
			ctx.effect(() => ctx.slots.inject("conversation.session.header.utilities", () => ctx.slots.register({
				...
				id: "dsh-conversation-rail-rail",
				order: 90,
				label
			}, ConversationRail)), "dsh-conversation-rail: conversation rail");
		}
		//#endregion
		exports.apply = apply;
